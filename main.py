...
if np.DataSource().exists("./model.h5"):
    ...
    model = train(features, labels, epochs=50,type='cnn2d', print_summary=True,
        save_model=True)
    model.save('./model.h5')

# step 3: prediction
while True:
    pred = predict(model, 'D:/Noise Class/pyAudioClassification-master/pyaudioclassification/output.wav')
